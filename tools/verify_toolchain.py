...
def python_module_ok(module: str) -> bool:
...
def main() -> int:
    ...
    for cmd in [
        "git", "curl", "unzip", "file", "cmake", "ninja", "clang", "llvm-objdump",
        "aarch64-linux-gnu-objdump", "aarch64-linux-gnu-readelf", "java", "zstd", "lz4",
    ]:
    ...
    ghidra = TOOLS / "ghidra"
    ghidra_ok = (ghidra / "ghidraRun").is_file() and (ghidra / "support" / "analyzeHeadless").is_file()
    checks.append(("ghidra", ghidra_ok, str(ghidra)))
    ...
    build_ok = build_dir.is_dir() and any(build_dir.iterdir())
    checks.append(("ryubing-build", build_ok, str(build_dir)))
