...
def main() -> int:
    ...
    lock = json.loads(LOCK.read_text(encoding="utf-8")); checks: list[tuple[str,bool,str]] = []
    ...
    switch_loader = TOOLS / "src" / "Ghidra-Switch-Loader"; head = git_head(switch_loader)
    checks.append(("switch-loader-commit", head == lock["switch_loader"]["commit"], head or "not cloned"))
    ext_root = ghidra / "Ghidra" / "Extensions"
    ext_ok = ext_root.is_dir() and any("switch" in p.name.lower() for p in ext_root.iterdir())
    checks.append(("switch-loader-installed", ext_ok, str(ext_root)))
    ...
    if py.is_file():
        for label,module in {"capstone":"capstone","lief":"lief","pyelftools":"elftools","construct":"construct","kaitaistruct":"kaitaistruct","rich":"rich","xxhash":"xxhash"}.items():
            rc = subprocess.run([str(py), "-c", f"import {module}"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode
            checks.append((f"python:{label}", rc == 0, module))
    ...
    for name,ok,detail in checks:
        print(f"{name:<{width}}  {'OK' if ok else 'MISSING':<7}  {detail}"); failures += int(not ok)
    print(f"\nchecks={len(checks)} failures={failures}")
    return 0 if failures == 0 or args.allow_partial else 1
