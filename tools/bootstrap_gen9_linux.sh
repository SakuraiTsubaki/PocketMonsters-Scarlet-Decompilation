...
if [ "${GEN9_SKIP_SYSTEM:-0}" != "1" ]; then
  if command -v apt-get >/dev/null 2>&1; then
    ...
    run_root apt-get install -y \
      git curl ca-certificates unzip jq file xxd build-essential make cmake ninja-build \
...
export PATH="$BIN_DIR:$DOTNET_DIR:$PATH"
...
if [ "${GEN9_SKIP_SWITCH_TOOLS:-0}" != "1" ]; then
  ...
  cp "$SRC_DIR/hactool/config.mk.template" "$SRC_DIR/hactool/config.mk"
  make -C "$SRC_DIR/hactool"
  install -m 0755 "$SRC_DIR/hactool/hactool" "$BIN_DIR/hactool"
  ...
  if [ -x "$GHIDRA_DIR/ghidraRun" ]; then
    ...
    ext_zip="$(find "$SRC_DIR/Ghidra-Switch-Loader/dist" -maxdepth 1 -type f -name '*.zip' | head -n1)"
    test -n "$ext_zip"
    mkdir -p "$GHIDRA_DIR/Ghidra/Extensions"
    first_entry="$(unzip -Z1 "$ext_zip" | head -n1)"
    if [[ "$first_entry" == Ghidra/Extensions/* ]]; then
      unzip -q -o "$ext_zip" -d "$GHIDRA_DIR"
    else
      unzip -q -o "$ext_zip" -d "$GHIDRA_DIR/Ghidra/Extensions"
    fi
  fi
fi
